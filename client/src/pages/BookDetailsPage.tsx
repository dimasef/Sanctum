...
function BookDetailsPage() {
  ...
  const reading = useQuery(MY_READING, { skip: status !== 'authenticated' });

  const [setReadingStatus, setState] = useMutation(SET_READING_STATUS, {
    refetchQueries: [{ query: MY_READING }],
  });
  const [removeReadingStatus, removeState] = useMutation(REMOVE_READING_STATUS, {
    refetchQueries: [{ query: MY_READING }],
  });
  ...
  const currentStatus =
    reading.data?.me?.readingStatuses.find((item) => item.book.id === id)?.status ?? null;
  const busy = setState.loading || removeState.loading;
  const blurb = stripHtml(book.description);

  const handleStatus = async (next: ReadingState) => {
    ...
    setShelfError(null);
    try {
      await setReadingStatus({ variables: { bookId: id, status: next } });
    } catch (err) {
      setShelfError(err instanceof Error ? err.message : 'Could not update your reading status.');
    }
  };
  ...
  const handleRemove = async () => {
    setShelfError(null);
    try {
      await removeReadingStatus({ variables: { bookId: id } });
    } catch (err) {
      setShelfError(err instanceof Error ? err.message : 'Could not update your reading status.');
    }
  };
        ...
        <InfoPanel>
          ...
          <Box sx={{ mt: 3 }}>
            <ToggleButtonGroup
              ...
              value={currentStatus}
              disabled={busy}
              onChange={(_event, value: ReadingState | null) => value && handleStatus(value)}
            >
              {STATUS_ORDER.map((s) => (
            ...
            {status === 'authenticated' && (
              <Box sx={{ mt: 1.5 }}>
                <AddToShelfControl bookId={book.id} />
              </Box>
            )}
